...
object KafkaSparkCassandra {
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setMaster("local[2]").setAppName("KafkaSparkCassandra")

    // get the values we need out of the config file
    val kafka_broker = "localhost:9092"
    val kafka_topic = "test2"
    val cassandra_host = "127.0.0.1" 
    val cassandra_user = "cassandra"
    val cassandra_pass = "cassandra"
    ...
    sparkConf
      ...
      //Cassandra Connector instead of directly from the driver
